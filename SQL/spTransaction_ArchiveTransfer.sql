...
BEGIN
    ...
    START TRANSACTION;
		...
		IF (rowCountTransactionCredit > 0 AND rowCountTransactionDebit > 0 AND 
            ...
            CALL spBankTransactionHistory_GenerateById(transactionUpdatedBy, varFromBankDebit);
            CALL spBankTransactionHistory_GenerateById(transactionUpdatedBy, varFromBankCredit);
			COMMIT;
		ELSE
			ROLLBACK;
		END IF;
